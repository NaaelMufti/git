import java.util.*;
import java.io.*; //input and output (files)
...
public class CountryTest
{
	...
	public static void loadCountries() throws IOException // throws IOException -> whenever finding file, needs try catch
	{	// class worksheets, notes and tests
		countries.clear();
		File file = new File("country-codes.txt");
		FileReader fr = new FileReader(file); //setting to read mode instead of write mode
		BufferedReader read = new BufferedReader(fr); //buffer where bites go, always returns string after reading line
		String line = "";
		while( read.ready() )
		{
			line = read.readLine();
	...
	public static void main(String[] args) throws IOException // call something that throws exception, so must throw exception
